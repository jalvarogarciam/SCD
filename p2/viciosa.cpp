...
const int num_fumadores = 3;   //   núm. de fumadores
...
{/*Los ingredientes son tabaco, papel y cerillas, que se representan con los enteros 0, 1, 2*/
 ...
 public:
   Estanco(){
      mostrador = -1 ; //inicialmente el mostrador está vacío
    ...
    */
   void  obtenerIngrediente( int ih){
      //si el ingrediente que necesita no está en el mostrador, espera
   ...
   void ponerIngrediente( int ingre ){
      mostrador = ingre;         //pone el ingrediente
      ingr_disp[ingre].signal(); //lo notifica
   }
