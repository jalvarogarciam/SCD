...
private:
 ...
 public:
     ...
     */
    void ini_lectura(int ih){
        if (escrib) lectura.wait();// si hay escritor, espera
        if (leyo[ih]) lectura.wait(); //si ya ha leido, espera
     ...
     * Usado solo por lectores
     */
    void fin_lectura(int ih){
        n_lec--;// registrar un lector menos

        leyo[ih] = true;// registra que ya ha leido
    ...
    void fin_escritura(void){
        ...
        for(int i=0; i<nl; i++) leyo[i]=false;  //registra que no se ha leido
...
void funcion_hebra_lectora( MRef<Lec_Esc> biblioteca , int ih)
{
    while (true){
        ...
        leer(ih);                   // lee
        biblioteca->fin_lectura(ih) ; // termina de leer
    }
}
// -----------------------------------------------------------------------------
