...
const long m  = 1024l*1024l*1024l, // número de muestras (del orden de mil millones)
           n  = 16  ;               // número de hebras concurrentes (divisor de 'm')
...
double funcion_hebra( long i )
{
   ...
   {  const double xj = double(i+j*n+0.5)/m ;  //      calcular $x_j$
      suma += f( xj );                         //      añadir $f(x_j)$ a la suma actual
   }
   return suma/m ;                             // devolver valor promedio de $f$
}
...
double calcular_integral_concurrente( )
{
   ...
   for( int i = 0 ; i < n ; i++ )
      suma += futuros[i].get();  //get() devuelve los valores del future
